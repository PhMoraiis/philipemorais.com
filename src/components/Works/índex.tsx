...
const Works = ({ className, ...props }: CardProps) => {
  ...
  const getImage = (project: { image: string; imageDark: string }) => {
    ...
    if (project.imageDark === '.' || project.imageDark === '' || project.imageDark === null) {
  ...
  const getImageMobile = (project: { imageMobile: string, imageDarkMobile: string }) => {
    // Verifica se project.imageDark contém apenas um ponto
    if (project.imageDarkMobile === '.' || project.imageDarkMobile === '' || project.imageDarkMobile === null) {
  ...
  const mobileImageBG = (project: { imageMobile: string, imageDarkMobile: string }) => ({
  ...
  const transformStatus = (status: string) => {
    switch (status) {
      case 'ONLINE':
        return { label: 'Online', labelEnglish: 'Online', bgColor: 'bg-[#00eb4e]', hoverClass: isHover ? 'neon2' : 'neon' }
      case 'DESENVOLVIMENTO':
        return { label: 'Desenvolvimento', labelEnglish: 'Development', bgColor: 'bg-[#4D4DFF]', hoverClass: isHover ? 'neonDev1' : 'neonDev2' }
      case 'INTERROMPIDO':
        return { label: 'Interrompido', labelEnglish: 'Stopped', bgColor: 'bg-[#DEFF1C]', hoverClass: isHover ? 'neonStop1' : 'neonStop2' }
      default:
        return { label: 'Online', bgColor: 'bg-[#00eb4e]', hoverClass: isHover ? 'neon2' : 'neon' }
    }
  }
      ...
      {projects.map((project) => {
        ...
        return (
          <Card key={project.id} onClick={() => handleHref(project.href)} className={cn('w-full md:w-full md:h-[330px] lg:w-full lg:h-[380px] h-[550px] shadow-xl rounded-xl cursor-pointer hidden sm:flex', className)} {...props} style={imageBG({ image: project.image, imageDark: project.imageDark })}>
            <CardHeader>
              <CardTitle className={cn('text-secondary', { 'text-[#333333] dark:text-secondary': (projects.indexOf(project) === 1 || projects.indexOf(project) === 2) && !isDark })}>
                {project.name}
              </CardTitle>
              <CardDescription className={cn('text-secondary', { 'text-[#333333] dark:text-secondary': (projects.indexOf(project) === 1 || projects.indexOf(project) === 2) && !isDark })}>
                {description}
              </CardDescription>
            </CardHeader>
            <CardFooter className='gap-3'>
              <button type='button' className={'inline-flex items-center rounded-full border px-3 py-1 gap-2 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 border-transparent bg-primary text-primary-foreground hover:bg-primary/80'} onMouseLeave={handleMouseLeave} onMouseEnter={handleMouseEnter}>
                <div className={`rounded-full w-3 h-3 ${statusInfo.bgColor} mr-2 ${isHover} duration-300 ease-in-out`} />
                {statusLabel}
              </button>
      ...
      {projects.map((project) => {
        ...
        return (
          <Card
            ...
            style={mobileImageBG({ imageMobile: project.imageMobile, imageDarkMobile: project.imageDarkMobile })} // Passando a propriedade correta
            {...props}
          ...
          >
            ...
            <CardFooter className='gap-3 flex-wrap'>
              <button type='button' className="inline-flex items-center rounded-full border px-3 py-1 gap-2 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 border-transparent bg-primary text-primary-foreground hover:bg-primary/80" onMouseLeave={handleMouseLeave} onMouseEnter={handleMouseEnter}>
                <div className={`rounded-full w-3 h-3 ${statusInfo.bgColor} mr-2 ${statusInfo.hoverClass} duration-300 ease-in-out`} />
                {statusLabel}
              </button>
              {project.techs.map((tech) => (
                <Badge key={tech.id} size="icon">
