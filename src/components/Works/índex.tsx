      ...
      {projects.map((project) => {
        ...
        return (
          <Card key={project.id} onClick={() => handleHref(project.href)} className={cn('w-full md:w-full md:h-[330px] lg:w-full lg:h-[380px] h-[550px] shadow-xl rounded-xl cursor-pointer hidden sm:flex', className)} {...props} style={imageBG({ image: project.image, imageDark: project.imageDark })}>
            <CardHeader>
              <CardTitle className={cn('text-primary', { 'text-[#333333]': (projects.indexOf(project) === 1 || projects.indexOf(project) === 2) && !isDark })}>
                {project.name}
              </CardTitle>
              <CardDescription className={cn('text-primary', { 'text-[#333333]': (projects.indexOf(project) === 1 || projects.indexOf(project) === 2) && !isDark })}>
                {description}
              </CardDescription>
            </CardHeader>
