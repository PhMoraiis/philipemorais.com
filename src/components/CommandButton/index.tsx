			...
			<CommandDialog open={open} onOpenChange={setOpen}>
				<CommandList className='overflow font-Relative'>
					...
					<CommandGroup heading={t('CommandGroup5')}>
						<CommandItem
							className={`flex justify-between ${activeLocale === 'en' ? 'bg-accent' : ''}`}
						>
							<Button
								variant='noHover'
								size='sm'
							...
							{activeLocale === 'en' && (
								<CommandShortcut className='text-lg p-2 bg-secondary-foreground dark:bg-secondary-foreground rounded-lg'>
									<Check className='h-4 w-4' />
								</CommandShortcut>
							)}
						</CommandItem>
						<CommandItem
							className={`flex justify-between ${activeLocale === 'pt-br' ? 'bg-accent' : ''}`}
						>
							<Button
								variant='noHover'
								size='sm'
